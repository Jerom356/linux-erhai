...
static void pogo_keyboard_plat_remove(struct platform_device *device);
...
static inline int fb_kevent_send_to_user(void *info)
{
    return 0;
}
...
static void pogo_keyboard_heartbeat_switch(int value)
{
...
static void pogo_keyboard_plug_switch(int value)
{
...
static void pogo_keyboard_poweroff_timer_switch(bool state)
{
...
static void pogo_keyboard_plugin_check_timer_switch(bool state)
{
...
//获取crc16
static unsigned short app_crc16_get(unsigned char *buf, unsigned short len, unsigned char crc_type)
{
    unsigned char i = 0;
...
static int uart_package_data(char *buf, int len, unsigned char *p_out, int *p_len)
{
    unsigned short i = 0;
...
static bool pogo_keyboard_data_is_valid(char *buf, int len)
{
    char flag = 0;
    unsigned short crc16 = 0;
    if (len < 5) {
...
static int pogo_keyboard_store_recv_data(char *buf, int len)
{
    int ret = 0;
...
static int pogo_keyboard_recv(char *buf, int len)
{
...
static int pogo_keyboard_recv_callback(char *buf, int len)
{
    return pogo_keyboard_recv(buf, len);
}

static int pogo_keyboard_get_valid_data(char *buf, int len, unsigned char *out_buf, int *out_len)
{
    int count = 0;
...
static ssize_t tty_write_ex(const char *buf, size_t count)
{
    ssize_t ret = 0;
...
static int  pogo_keyboard_tp_ver(void)
{
...
static int  pogo_keyboard_ver(void)
{
...
//TRX data test
static int  pogo_keyboard_trx_test(char *write_buf, u8 write_len, char *read_buf, u8 read_len)
{
...
static int pogo_keyboard_get_charge_current(void)
{
...
static bool pogo_keyboard_is_support(struct uart_port *port)
{
    bool ret = false;
...
static int pogo_keyboard_get_dts_info(struct platform_device *pdev)
{
    ...
    ret = gpio_request_one(pogo_keyboard_client->uart_wake_gpio, GPIOD_IN, "uart_wake_gpio");
    if (ret) {
...
static int pogo_keyboard_write_callback(void *param, int enable)
{
    if (enable == 1) {
...
static int pogo_keyboard_init_callback(void *port, int type)
{
    struct file *filp;
...
static struct pogo_keyboard_operations *get_pogo_keyboard_operations(void)
{
    return NULL;
}

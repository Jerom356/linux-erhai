/*
 * Copyright (c) 2024, OnePlus Technology (Shenzhen) Co., Ltd. All rights reserved.
 * Copyright (c) 2026, Jerom356
 *
...
// Below are functions originally defined in a header file
enum POWER_GPIOS{
	KTZ8868_HW_EN,    //GPIO62
	LCD_PWR_GPIO_MAX
};
struct ktz8868_gpio_info {
	char* gpio_name;
	int gpio_num;
};
int bl_ic_ktz8868_hw_en(bool on);
int bl_ic_ktz8868_set_brightness(int value);
int bl_ic_ktz8868_set_lcd_bias_by_gpio(bool enable);
int __init bl_ic_ktz8868_init(void);
void __exit bl_ic_ktz8868_exit(void);

// Use a gamma curve instead of pre-defined backlight_map[] array
unsigned int backlight_map(unsigned int brightness)
{
    return (unsigned int)(2047.0 * pow((double)brightness / (double)N, 0.45));
}
